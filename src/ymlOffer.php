...
class ymlOffer extends \DomElement
{
    ...
    protected function _age($args)
    {
        ...
        $ageEl = new \DomElement('age', htmlspecialchars($args[0]));
    ...
    {
        $newEl = new \DomElement('param', htmlspecialchars($args[1]));
    ...
    {
        $newEl = ($val === false) ? new \DomElement($name) : new \DomElement($name, htmlspecialchars($val));
        $this->appendChild($newEl);
        return $this;
    }
